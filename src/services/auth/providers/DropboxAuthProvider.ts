...
export class DropboxAuthProvider implements AuthProvider {
  ...
  async login(): Promise<void> {
    ...
    const params = new URLSearchParams({
      ...
      state: 'dropbox', // Important for callback identification
    });
  ...
  async handleCallback(): Promise<boolean> {
    ...
    if (!code || state !== 'dropbox' || this.isExchanging) return false;
    ...
    try {
      ...
      console.error('[DropboxAuth] handleCallback error:', err);
      this.isExchanging = false;
      return false;
    }
  }
  ...
  async getAccessToken(): Promise<string> {
    ...
    throw new Error('Dropbox token unavailable');
  }
  ...
  private async refreshAccessToken(): Promise<void> {
    ...
    if (!response.ok) {
      this.logout();
      throw new Error('Dropbox session expired');
    }
